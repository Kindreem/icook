  ...
  <div class="meet">
    <header class="header">
     ...
     <div class="tit">选择您拿手的或想做的<p>最多可选4个</p></div>
      <div class="search">
         <input type="text" placeholder="输入菜系或菜谱" v-model="search"
        @input="handleQuery">
         <div class="icon"><img src="../../assets/images/DR-019.png" alt="" @click="clear"></div>
         <div class="sel" v-show="show">
           <ul>
             <li v-for="(item,index) in sel" :key="index" @click="add(index)" v-html="item.cbname">
               <!-- {{item.name}} -->
             </li>
           </ul>
         </div>
    ...
    <ul class="greens">
       <li v-for="(item,index) in names" :key="index" @click="onshow(index)" :style="{background:item.colour}">
         <p v-html="item.cbname"></p>
         <span class="checkbox"><input type="radio" :checked="item.ishow"></span>
         <div id="triangle-topright"></div>
         <h4>{{item.factionsname}}</h4>
       </li>
    </ul>
      <div class="btn-meet">
        <p>完成奖励 : 完成后可获得 徽章</p>
...
import { likesearch, likelist } from "@/api";
export default {
  data() {
    return {
      show: false,
  ...
  methods: {
    ...
    add(id) {
      ...
      if (checked.length < 4) {
        this.names.unshift({
          colour: this.sel[id].colour,
          cbname: this.sel[id].cbname,
          ishow: 1,
          factionsname: this.sel[id].factionsname
        });
      } else {
        //超出限制
        this.warning = 1;
...
.meet {
  ...
  .search {
    ...
    .sel {
      ...
      border-left: 1px solid #5db8e9;
      border-right: 1px solid #5db8e9;
      border-bottom:1px solid #5db8e9;
  ...
  .greens {
    ...
    li {
      ...
      #triangle-topright {
        ...
        width: 0;
        height: 0;
        border-top: px2rem(35) solid #fff;
        border-left: px2rem(37) solid transparent;
